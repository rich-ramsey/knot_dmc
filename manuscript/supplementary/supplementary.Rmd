...
(ref:descriptive-caption) Descriptive statistics for accuracy and reaction time data by training day and condition.

```{r descriptive-table}
descriptives <- read_csv("../../tables/manifest/descriptives.csv") |>
  ## create a factor and replace repeated variable terms
  mutate(
    day = as_factor(str_remove(day, "Day ")),
    variable = if_else(
      variable != lag(variable, default = ""),
      variable,
      ""
    )
  )
...
apa_table(
  descriptives,
  caption = "(ref:descriptive-caption)"
) 
```
...
(1 + condition * day | pid)$$ 
*Note:* RT = reaction time (ms); acc= accuracy (0,1); condition = training condition (untrained vs. naming vs. tying vs. both); day = day of testing session (day 1 vs. day 5); pid = subject/participant identifier; ndt = non-decision time.
...
(ref:prior-post-caption) Prior and Posterior Graphs. Red lines represent priors, while black lines represent posteriors. Only location parameters from day 5 are shown here.

```{r prior-post-plot, fig.cap = "(ref:prior-post-caption)"}
knitr::include_graphics("../../figures/ea/day5_pp_location.jpeg")
```
...
knitr::include_graphics("../../figures/ea/day5_KN_fit.pdf")
```

### Point and interval summary data
...
(ref:group-lba-table-caption) Point (median) and interval (95% quantile) estimates by training day and condition.

```{r group-lba-table}
group_q <- read_csv("../../tables/ea/group_q.csv") |>
  ## create a factor and replace repeated variable terms
  mutate(
    day = factor(day),
    parameter = if_else(
      parameter != lag(parameter, default = ""),
      parameter,
      ""
    )
  )
...
apa_table(
  group_q,
  caption = "(ref:group-lba-table-caption)"
)
```

(ref:contrasts-lba-table-caption) Point (median) and interval (95% quantile) estimates for pairwise contrasts between conditions on day 1 and day 5.

```{r contrasts-lba-table}
contrasts_q <- read_csv("../../tables/ea/contrasts_q.csv") |> 
  ## create a factor and replace repeated variable terms
  mutate(
    day = factor(day),
    parameter = if_else(
...
apa_table(
  contrasts_q,
  caption = "(ref:contrasts-lba-table-caption)",
  longtable = TRUE
) 
```

(ref:day-contrasts-lba-table-caption) Point (median) and interval (95% quantile) estimates for pairwise contrasts between pre-training and post-training across training conditions.

```{r day-contrasts-lba-table}
day_contrasts_q <- read_csv("../../tables/ea/day_contrasts_q.csv") |>
  ## replace repeated variable terms
  mutate(
    parameter = if_else(
      parameter != lag(parameter, default = ""),
      parameter,
...
apa_table(
  day_contrasts_q,
  caption = "(ref:day-contrasts-lba-table-caption)"
) 
```
