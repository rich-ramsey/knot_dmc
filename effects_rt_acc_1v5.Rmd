...
This file visualises and tabulates parameters in the most complex model from the manifest analysis of the knots data (reaction time and accuracy).

This file looks at effects from models with the day 1 (pre-training) and day 5 (post-training) data.
...
post_summary <- posterior_summary(b3, robust = TRUE)
...
# I take this to be 18 pts x 9 terms (ndt_intercept, intercept, 3 levels of group, 1 level of day, 3 intx terms) = 162 in total
vary_vals <- post %>%
...
tidy_fixed <- post %>% 
  ...
  mutate(key = factor(name, levels=unique(name)),
         term = if_else(str_detect(name, "ndt"), "ndt_intercept",
                if_else(str_detect(name, "both:DAYDay5"), "both*day5",
                if_else(str_detect(name, "tie:DAYDay5"), "tie*day5",
                if_else(str_detect(name, "name:DAYDay5"), "name*day5",
                if_else(str_detect(name, "name"), "name",
                if_else(str_detect(name, "tie"), "tie",
                if_else(str_detect(name, "both"), "both", 
                if_else(str_detect(name, "Day5"), "day5","intercept")))))))),
...
p_tidy_fixed <- ggplot(tidy_fixed, aes(x = value, y = (term), 
                                       fill=term)) +  
...
p_tidy_varying_pid <- ggplot(tidy_varying, aes(x = value, y = term)) +  
  geom_vline(xintercept = 0, color = "grey", alpha = 5/10) +
  stat_halfeye(alpha = 0.7) +
...
ggsave ("figures/manifest/day1v5/b1_varying_pid.jpeg",
        width = 8, height = 6, dpi = 800)
```
...
mutate(key = factor(param, levels=unique(param)),
         term = if_else(str_detect(param, "ndt"), "ndt_intercept",
                if_else(str_detect(param, "both:DAYDay5"), "both*day5",
                if_else(str_detect(param, "tie:DAYDay5"), "tie*day5",
                if_else(str_detect(param, "name:DAYDay5"), "name*day5",
                if_else(str_detect(param, "name"), "name",
                if_else(str_detect(param, "tie"), "tie",
                if_else(str_detect(param, "both"), "both", 
                if_else(str_detect(param, "Day5"), "day5","intercept")))))))),
         term = factor(term, levels=c("ndt_intercept", "intercept",
                                      "name", "tie", "both", "day5",
                                      "name*day5", "tie*day5", "both*day5"))) |> 
  arrange(term, Estimate) %>% 
  mutate(rank = rep(1:18, times=9),
...
ggsave ("figures/manifest/day1v5/b1_varyvals.jpeg",
        width = 8, height = 6, dpi = 800)
```
...
ppred_rt_tab <- ppreds_rt_q %>%
  select(DAY, GROUP, .epred, .lower, .upper) %>% 
  rename(condition = GROUP, day = DAY) |> 
...
```{r}
ab3_post <- as_draws_df(ab3) %>% #
  select(-lp__) %>% 
  as_tibble()

ab3_post_summary <- posterior_summary(ab3, robust = TRUE)

# all fixed effects
ab1_post_qi_b <- ab3_post %>%
  select(starts_with("b_")) %>% 
  pivot_longer(everything()) %>% 
  group_by(name) %>% 
  median_qi(value)
...
# varying effects by participant 
# I take this to be 18 pts x 8 terms (intercept, 3 levels of group, 1 level of day, 3 intx terms) = 144 in total
ab3_vary_vals <- ab3_post %>%
  select(starts_with("r_")) %>% 
  pivot_longer(everything()) %>% 
  group_by(name) %>% 
  median_qi(value)
...
## fixed effects of interest, excluding basic intercepts 
## Is this the right way to label the interaction?
ab3_tidy_fixed <- ab3_post %>% 
  select(starts_with("b_")) %>% # select and rename in simpler labels.
  pivot_longer(everything()) %>% # move from wide to long
  mutate(key = factor(name, levels=unique(name)),
         term = if_else(str_detect(name, "both:DAYDay5"), "both*day5",
                if_else(str_detect(name, "tie:DAYDay5"), "tie*day5",
                if_else(str_detect(name, "name:DAYDay5"), "name*day5",
                if_else(str_detect(name, "name"), "name",
                if_else(str_detect(name, "tie"), "tie",
...
## check labels
check.labels <- ab3_tidy_fixed %>% 
  group_by(key, term) %>% 
  distinct(key, term)
check.labels

# remove intercepts as they are on a different scale and make it difficult to plot
ab3_tidy_fixed <- ab3_tidy_fixed %>%
...
p_tidy_fixed_ab3 <- ggplot(ab3_tidy_fixed, aes(x = value, y = term, 
                                       fill=term)) +  
  geom_vline(xintercept = 0, color = "grey", alpha = 5/10) +
  stat_halfeye(alpha=0.7) +
  labs(#title = "fixed effects of condition",
...
ab3_tidy_varying <- ab3_post %>% 
  ...
  mutate(key = factor(name, levels=unique(name)),
         ...
         term = factor(term, levels=c("intercept",
                                      "name", "tie", "both", "day5",
                                      "name*day5", "tie*day5", "both*day5")))
head(ab3_tidy_varying)
tail(ab3_tidy_varying)
str(ab3_tidy_varying)

check.varying <- ab3_tidy_varying %>% 
  group_by(key, term) %>% 
  distinct(key, term)
check.varying

p_tidy_varying_pid_ab3 <- ggplot(ab3_tidy_varying, aes(x = value, 
                                               y = term)) +  
  geom_vline(xintercept = 0, color = "grey", alpha = 5/10) +
  stat_halfeye(alpha = 0.7) +
  labs(title = "SD plot for varying participant effects",
  ...
  scale_x_continuous(breaks=seq(0,1,0.1))
p_tidy_varying_pid_ab3

ggsave ("figures/manifest/day1v5/ab3_varying_pid.jpeg",
        width = 8, height = 6, dpi = 800)
```
...
# participant level
ab3_vary_pt <- ab3_post_summary %>%
  data.frame() %>% 
  rownames_to_column("param") %>% 
  filter(str_detect(param, "r_PID"),
         str_detect(param, "cor", negate = TRUE)) %>% 
  mutate(key = factor(param, levels=unique(param)),
         term = if_else(str_detect(param, "both:DAYDay5"), "both*day5",
                if_else(str_detect(param, "tie:DAYDay5"), "tie*day5",
                if_else(str_detect(param, "name:DAYDay5"), "name*day5",
                if_else(str_detect(param, "name"), "name",
                if_else(str_detect(param, "tie"), "tie",
...
```{r}
p_tidy_varyvals_ab3 <- ggplot(ab3_vary_pt, aes(x=rank, y=Estimate,
                                           colour=outlier))+
  geom_pointrange(aes(ymin=Q2.5,ymax=Q97.5),fatten = 0.5)+
  geom_hline(yintercept=0, colour = "red")+
  scale_colour_manual(values=c("darkgrey","black")) +
...
ggsave ("figures/manifest/day1v5/ab3_varyvals.jpeg",
        width = 8, height = 6, dpi = 800)
```
...
```{r}
p_fixef <- (p_tidy_fixed_ab3 | p_tidy_fixed) +
...
ggsave("figures/manifest/day1v5/fixef.jpeg",
...
ggsave("figures/manifest/day1v5/ppreds.jpeg",
       width = 10, height = 6, dpi = 800)
```
...
write_csv(fix_tab, "tables/manifest/day1v5/fixef.csv")
```
