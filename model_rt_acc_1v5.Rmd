...
It uses the day 1 (pre-training) and the day 5 (post-training) data.
...
## checks
## day 5, session 2 AND Day 1, session 1
data_acc %>%
...
b0.1 <- brm(formula = formula,
        data = data_rt, family = shifted_lognormal(),
        prior = priors,
        iter = 2000, warmup = 1000, cores = 28, chains = 4,
        save_pars = save_pars(all=TRUE),
        seed = 123,
        file = "models/manifest/day1v5/b0.1")
summary(b0.1)
```
...
b0.2 <- brm(formula = formula,
        data = data_rt, family = shifted_lognormal(),
        prior = priors,
        iter = 4000, warmup = 2000, cores = 28, chains = 4,
        save_pars = save_pars(all=TRUE),
        seed = 123,
        file = "models/manifest/day1v5/b0.2")
summary(b0.2)
```
...
b0.3 <- brm(formula = formula,
        ...
        save_pars = save_pars(all=TRUE),
        seed = 123,
        file = "models/manifest/day1v5/b0.3")
summary(b0.3)
```
...
b1 <- brm(formula = formula,
        ...
        seed = 123,
        init = 0.1,
        file = "models/manifest/day1v5/b1")
summary(b1)
```
...
### model b2 - add DAY (pre-training vs post-training) as a predictor ###

formula

```{r}
formula = bf(RT ~ 1 + GROUP + DAY +
             (1 + GROUP + DAY | PID),
             ndt ~ (1 | PID))
```
...
b2 <- brm(formula = formula,
        ...
        seed = 123,
        init = 0.1,
        file = "models/manifest/day1v5/b2")
summary(b2)
```
...
```{r}
# p1 = plot(conditional_effects(b2), plot=FALSE)
# 
# wrap_plots(p1)
```
...
```{r}
ppb2 <- pp_check(b2, ndraws = 100)
ppb2
```

### model b3 - add GROUP*DAY interaction as a predictor ###
...
b3 <- brm(formula = formula,
        ...
        seed = 123,
        init = 0.1,
        file = "models/manifest/day1v5/b3")
summary(b3)
```
...
```{r}
# p1 = plot(conditional_effects(b3), plot=FALSE)
# 
# wrap_plots(p1)
```
...
ab0.1 <- brm(formula = formula,
        ...
        save_pars = save_pars(all=TRUE),
        seed = 123,
        file = "models/manifest/day1v5/ab0.1")
summary(ab0.1)
```
...
ab0.2 <- brm(formula = formula,
        ...
        save_pars = save_pars(all=TRUE),
        seed = 123,
        file = "models/manifest/day1v5/ab0.2")
summary(ab0.2)
```
...
ab1 <- brm(formula = formula,
        ...
        save_pars = save_pars(all=TRUE),
        seed = 123,
        file = "models/manifest/day1v5/ab1")
summary(ab1)
```
...
### model ab2 - add DAY (training session, pre vs post-training) as a predictor ###

formula

```{r}
formula = bf(ACC ~ 1 + GROUP + DAY +
             (1 + GROUP + DAY | PID))
```
...
```{r}
plan(multicore)
ab2 <- brm(formula = formula,
        data = data_acc, family = bernoulli(link = "logit"),
        prior = priors,
        iter = 4000, warmup = 2000, cores = 28, chains = 4,
        save_pars = save_pars(all=TRUE),
        seed = 123,
        file = "models/manifest/day1v5/ab2")
summary(ab2)
```
...
```{r}
ppab2 <- pp_check(ab2, ndraws = 100)
ppab2
```

### model ab3 - add the GROUP * DAY interaction as a predictor ###
...
```{r}
plan(multicore)
ab3 <- brm(formula = formula,
        data = data_acc, family = bernoulli(link = "logit"),
        prior = priors,
        iter = 4000, warmup = 2000, cores = 28, chains = 4,
        save_pars = save_pars(all=TRUE),
        seed = 123,
        file = "models/manifest/day1v5/ab3")
summary(ab3)
```
...
p_chains1
# save it
ggsave ("figures/manifest/day1v5/b3_chains.jpeg",
        width = 8, height = 6)
```
...
```{r}
b3_neff <- mcmc_plot(b3, type = "neff")
b3_neff

b3_rhat <- mcmc_plot(b3, type = "rhat")
b3_rhat

# this creates a combined plot
b3_diag <- b3_neff / b3_rhat 
b3_diag

ggsave("figures/manifest/day1v5/b3_diag.jpeg",
       width = 8, height = 6)
```
...
p_chains2
# save it
ggsave ("figures/manifest/day1v5/ab3_chains.jpeg",
        width = 8, height = 6)
```
...
```{r}
ab3_neff <- mcmc_plot(ab3, type = "neff")
ab3_neff

ab3_rhat <- mcmc_plot(ab3, type = "rhat")
ab3_rhat

# this creates a combined plot
ab3_diag <- ab3_neff / ab3_rhat 
ab3_diag

ggsave("figures/manifest/day1v5/ab3_diag.jpeg",
       width = 8, height = 6)
```
