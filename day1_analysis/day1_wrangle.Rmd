...
This is the day 1 pre-test data wrangling file for the analysis of knots data from Cross et al. (2012).
...
This day 1 pre-test data analysis is a supplementary analysis in response to some reviewer comments.
...
```{r}
raw <- read_csv("data/processed/day1_pre_knots_data.csv")
head(raw)
```

check only day 1, session 1
...
```{r}
write_csv(data, "data/processed/day1_data_acc.csv")
write_csv(data_rt, "data/processed/day1_data_rt.csv")
```
...
```{r}
write_csv(data_lba, "data/processed/day1_data_lba.csv")
```
